...
name `autopilot-cluster-1` in the `us-east1` region, and left the networking and
...
add annotations specify our scrape port. Create another YAML file called
...
take snapshots either on-demand or on a specified schedule.
...
Finally, we can create a schedule using a `cron`-like string:
...
For more information on Valero, my article about
...
Postgres like TimescaleDB. Also, as a newer product, QuestDB is working on
enterprise features (e.g., replication, high-availability, recovery from
snapshot) and integrations with other popular projects. With Prometheus Postgres
exporter and Velero, we can configure a production-ready QuestDB on Kubernetes.
I look forward to enhancing this setup in future releases when more monitoring
and ops features will be added.

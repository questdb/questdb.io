...
```
docker run -p 9000:9000 -p 8812:8812 --name questdb questdb/questdb
```

By giving the container a name with `--name questdb`, we can reuse it at a later point and
the data that we have ingested will be persisted. If we want to re-run the same
image after killing the running docker container, we can use the following
commands:

---
title: Slack alerting on realtime stock price data with QuestDB and Grafana
author: Kovid Rathee
description:
...
[Kovid Rathee](https://kovidrathee.medium.com/) who written a great guide for
setting up Slack alerts via Grafana.
...
visualize the data and notify Slack on changes that interest us. We use Python
...
```bash
git clone git@github.com:bsmth/questdb-slack-grafana-alerts.git
cd questdb-slack-grafana-alerts
docker-compose up
```

Running `docker-compose up` brings up two containers that are networked:
...
This means you can use QuestDB as a default data source in Grafana right away
...
live stock prices, but first we can verify the configuration is correct by using
a test script which generates dummy data. The mock script generates random
...
type which is optimized for text columns that have repetitive values. More
information on this type can be found on the QuestDB
...
making requests to an API for live market data and query stock prices in
real-time. To configure IexFinance API authentication:
...
Now we can add this token to our project so the Python scripts have programmatic
access to the IexFinance API. To store project secrets, we are using the
[Python dotenv](https://pypi.org/project/python-dotenv/) package which allows
passing configuration to Python scripts as environment variables. We can
explicitly pass a token to each API call to IexFinance, or we can use the
...
If the markets are open, realtime prices will be sent to QuestDB:
...
The next step is to create an incoming webhook in Slack which we can sent alerts
as HTTP(S) requests from Grafana. To set up the webhook:

1. Navigate to [new Slack app](https://api.slack.com/apps?new_app=1) and create
   an app called **Price Alerts** for your workspace
2. Click **Incoming Webhooks** in the features section
3. Activate incoming webhooks and click **Add New Webhook to Workspace**
4. Select the Slack channel you want to receive alerts and click **Allow**
...
1. Log into Grafana at [localhost:3000](http://localhost:3000) using the default
   credentials:
...
5. In the **Notification settings** dropdown, check the **Include image**
   checkbox
6. Click **Test** to verify that the configuration is correct and click **Save**
...
Next up, we can create a new panel on Grafana and configure alerting based on
...
- `IS ...` allows specifying a comparison to a value or a range
...
1. Edit the query we built and select the **Alert** tab
...
In this tutorial, we have learned how Grafana and QuestDB can be quite powerful
for data visualization and alerting. We learned how to ingest live ticker data
from a REST API into QuestDB and how to visualize this data in a Grafana
dashboard and set up alerts based on predefined conditions.
...
[submit a PR](https://github.com/bsmth/questdb-slack-grafana-alerts) if you have

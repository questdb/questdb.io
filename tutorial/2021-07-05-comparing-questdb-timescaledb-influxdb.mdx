...
TimescaleDB, and InfluxDB for features, functionality, maturity and performance.
...
as money is flowing into the industry at historic rates (e.g.
...
decade, a new type of databases has been steadily growing in popularity.
...
store time-stamped data. Such data may include metrics from servers and
applications, readings from IoT sensors, user interaction on a website or an
app, or trading activity on financial markets.

Timeseries workloads are usually characterized by the following properties:

- Each data point includes a timestamp that can be used to index, aggregate, and
  sample. This data can also be multi-dimensional and correlated.
- High write-speed (ingestion) is preferred to capture data at high frequencies.
- Summarized view of the data (e.g. downsampled or aggregated view, trend lines)
  may provide more insight than a single data point. For example, given network
  unreliability or outliers in sensor readings, we may set alerts when some
  average value over time exceeds a threshold rather doing so on a single data
  point.
...
While other databases can also handle timeseries data to a certain extent, TSDBs
are designed with above properties in mind to handle data ingestion,
compression, and aggregation by time more efficiently. So as the demand for
timeseries data continues to explode on the heels of cloud computing, IoT, and
machine learning, how should architects go about choosing a TSDB? In this
article, we'll compare popular TSDBs as well as new player in the market to help
you decide.
...
for single node, with paid-plans available for InfluxDB Cloud and InfluxDB
...
Prior to the InfluxDB 2.x release in 2019, InfluxDB Platform consisted of the
...
primarily focused on timeseries data coming from servers and web applications.
Before Prometheus came along to take market share in this space, InfluxDB had
the largest community and integrations to collect, store, and view application
metrics.
...
InfluxDB 2.x largely simplified the architecture to bundle the TICK stack to a
single binary as well as introducing new features to make collecting (e.g.
...
- **Data schema (TSM & TSI):** data elements are stored in time-structured merge
  ...
  memory of the operating system. This is to help with datasets with high
  cardinality (i.e large elements in a set).
...
Most notably, InfluxDB does not enforce a schema before ingesting data. Rather
the schema is automatically created from the input data, inferred from the tags
and fields. This NoSQL-like experience is both InfluxDB's strength and weakness.
For datasets with relatively low cardinality that naturally fit this tagset
model (e.g. most infrastructure and application metrics, some IoT data, some
...
Finally, InfluxDB's decision to create its own custom functional data scripting
...
- Timeseries data line up with flow-based functional processing model where
...
- InfluxDB wanted first-class support for common operations with timeseries data
  (e.g. exponential moving average) that is not part of the SQL standard.

Flux syntax definitely requires some effort to adapt, especially if you're
looking for simple SQL queries or not looking to learn yet another new language.
Still considering the large community and
...
Overall, InfluxDB is a great choice if the timeseries data fit nicely with the
tagset model. The main use case seem to be geared towards
...
Image Credit: 2020 Stack Overflow Developer Survey

Because TimescaleDB began as an IoT platform, where they actually used InfluxDB
at first to store their sensor data, its features bode well for IoT timeseries
data which is often 'bursty', frequently out of order due to network
unreliability, and characterized by high cardinality:

- **Hypertables:** TimescaleDB
  [partitions its hypertables into chunks based on a time column as well as other "spatial" values](https://docs.timescale.com/timescaledb/latest/overview/core-concepts/hypertables-and-chunks/#partitioning-in-hypertables-with-chunks)
  such as a device uid, location identifier, or a stock symbol. These chunks can
  be configured to hold the most recent data in memory, asynchronously compress
  and re-order data by the time column to disk (instead of ingestion time), and
  replicate or migrate transactionally across nodes.
- **Continuous Aggregation:** TimescaleDB also supports
  ...
  temperature between 3pm and 4pm vs. what was the exact temperature at 3pm), so
  not needing to scan huge amount of data on every aggregate query can help
  create performant dashboards or analysis.
...
databases now have 2.x versions, so this analysis may be bit outdated, but the
...
influenced by data model, hardware, and configuration. Rather what this result
shows is that TimescaleDB may be better suited for IoT use cases where data
...
Overall, TimescaleDB is a great fit for teams who are looking for a big
...
numbers to reach top 5 TSDBs. Anecdotally, my previous IoT startup also used
TimescaleDB as an intermediate datastore to quickly pull aggregate metrics
spanning over several months and moved old data to long-term storage. Since we
were already running PostgreSQL on our Kubernetes cluster, installing
TimescaleDB and migrating our workload, was a simple task.

- **Pros:** PostgreSQL-compatibility, scales well with data cardinality, various
  deployment models available
- **Cons:** fixed schema (adds bit of complexity and data transformation effort
  prior to ingestion)

## QuestDB details

For those looking to take advantage of the flexibility of InfluxDB line protocol
and the familiarity of PostgreSQL, a newer timeseries database may satisfy both
...
By building the database from scratch with Java and C++, QuestDB team focused on
three things:
...
640k row/secs at 10 million devices using 4 threads on `m5.8xlarge` instance
with Intel Xeon CPUs. When the same benchmark was run on AMD Ryzen 3970X,
...
simply configure Telegraf to point to QuestDB's address and port. Likewise for
...
As a newer entrant in this space, the most obvious downside to QuestDB is lack
of support for production-ready features (e.g. replication, backup/restore). It
does integrate with some of the most popular tools already (e.g. PostgreSQL,
Grafana, Kafka, Telegraf, Tableau), but it will need some time to reach the
level of the other TSDBs mentioned above.
...
As the demand for timeseries data continues to grow, TSDBs specialized to deal
with this data will see massive adoption as well as fierce competition. Besides
the three open-source TSDBs covered in this article, there are also public cloud
...
performance is the main concern, QuestDB is a promising project that is growing
quickly.

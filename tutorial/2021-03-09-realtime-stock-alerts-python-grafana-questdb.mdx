---
title: Realtime stock price alerts using Python, Grafana and QuestDB
author: Kovid Rathee
description:
...
[Kovid Rathee](https://kovidrathee.medium.com/) who has written a great guide for
setting up alerting via Grafana.
...
[list supported of notifiers](https://grafana.com/docs/grafana/latest/alerting/notifications/#list-of-supported-notifiers).
...
realtime stock prices, but first, we can verify the configuration is correct by
...
## Query realtime stock prices

Once you have tested the ingestion using the mock data script, we can start
making requests to an API for live market data and query realtime stock prices.
...
The next step is to create an incoming webhook in Slack which to send alerts as
